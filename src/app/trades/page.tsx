    ...
    <div className="space-y-4 pb-6">
      ...
      {loading ? (
        ...
        <div className="card text-center py-12">
          ...
          <p className="text-muted text-xs mt-1">Trades appear here when orders are placed at 9:15 AM</p>
        </div>
      ) : (
              ...
              <div key={trade.id} className={`card border ${isOpen ? 'border-accent/20' : pnlPositive ? 'border-accent/10' : 'border-loss/10'}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    {pnlPositive ? <TrendingUp size={15} className="text-accent" /> : <TrendingDown size={15} className="text-loss" />}
                    <span className="text-data font-mono font-semibold text-sm">{trade.tradingSymbol}</span>
                    <span className={`badge ${trade.indexName === 'NIFTY' ? 'badge-blue' : 'badge-purple'}`}>{trade.indexName}</span>
                    <span className={`badge ${STATUS_COLORS[trade.status] || 'badge-muted'}`}>{trade.status}</span>
                    {trade.reentry && <span className="badge badge-amber">Re-entry</span>}
                    {trade.exitReason && <span className={`badge ${trade.exitReason === 'STOP_LOSS' ? 'badge-red' : 'badge-green'}`}>{EXIT_LABELS[trade.exitReason] || trade.exitReason}</span>}
                  </div>
                  <span className="text-muted text-[10px] font-mono shrink-0">#{trade.id}</span>
                </div>
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-3">
                  {[
                    { label: 'Entry', value: `₹${trade.entryPrice}`, cls: 'text-data' },
                    { label: 'Stop Loss', value: `₹${trade.stopLossPrice}`, cls: 'text-loss' },
                    { label: 'Target 1', value: `₹${trade.target1Price}`, cls: 'text-accent' },
                    { label: 'Target 2', value: `₹${trade.target2Price}`, cls: 'text-accent' },
                    { label: 'Exit', value: trade.exitPrice ? `₹${trade.exitPrice}` : '—', cls: 'text-data' },
                    { label: 'Qty', value: String(trade.quantity), cls: 'text-data' },
                  ].map(item => (
                    <div key={item.label} className="p-2 rounded-lg bg-surface-2">
                      <p className="text-muted text-[10px]">{item.label}</p>
                      <p className={`font-mono text-xs font-semibold mt-0.5 ${item.cls}`}>{item.value}</p>
                    </div>
                  ))}
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-white/5">
                  <div className="flex items-center gap-4 text-xs">
                    <div>
                      ...
                      <p className="text-data font-mono">{new Date(trade.entryTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</p>
                    </div>
                    {trade.exitTime && (
                      <div>
                        <p className="text-muted text-[10px]">Exit Time</p>
                        <p className="text-data font-mono">{new Date(trade.exitTime).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}</p>
                      </div>
                    )}
                    {trade.brokerOrderId && (
                      <div>
